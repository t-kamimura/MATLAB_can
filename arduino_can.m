...
ch = canChannel(arduinoObj,"Sparkfun CAN-Bus Shield", "BusSpeed", 125000);
...
cmd(1) = (bitshift(pos_tgt, -8)); % pos上8bitのみ抜き出し
cmd(2) = (bitand(pos_tgt, hex2dec('FF'))); % pos下8bitのみ抜き出し
cmd(3) = (bitshift(vel_tgt, -8)); % vel上8ビットのみ抜き出し
cmd(4) = (bitshift(bitand(vel_tgt, hex2dec('F')), 4) + bitand(bitshift(kp_tgt, -8), hex2dec('F'))); % velの下4桁と，kpの上4桁を合成
cmd(5) = (bitand(kp_tgt, hex2dec('FF'))); % kpの下8bit
cmd(6) = (bitshift(kd_tgt, -8)); % kdの上8bit
cmd(7) = (bitshift(bitand(kd_tgt, hex2dec('F')), 4) + bitand(bitshift(ff_tgt, -8), hex2dec('F'))); % kdの下4桁と，kpの上4桁を合成
cmd(8) = (bitand(ff_tgt, hex2dec('FF')));
